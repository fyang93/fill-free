...
import type { AppConfig, UploadedFile } from "bot/app/types";
...
export async function pruneMissingTelegramFileRecords(config: AppConfig): Promise<{ removed: number; removedPaths: string[] }> {
  const store = await readFilesStore(config.paths.repoRoot);
  ...
  for (const [key, record] of entries) {
    ...
    try {
      ...
      delete store.telegramFilesByUniqueId[key];
      removed += 1;
      removedPaths.push(record.savedPath);
    }
  }
  if (removed > 0) {
    await writeFilesStore(config.paths.repoRoot, store);
