import { storePendingAuthorizations } from "bot/operations/access/authorizations";
import { clearStoredUserAccessLevel, resolveStoredUserId, setStoredUserAccessLevel } from "bot/operations/access/roles";
...
export const accessSetRoleTaskHandler: TaskHandler = {
  ...
  run: async ({ config }, task) => {
    ...
    if (requestedRole === "allowed" || requestedRole === "trusted") {
      const changed = await setStoredUserAccessLevel(config, userId, requestedRole, { username, updatedBy: task.source.requesterUserId });
      return { result: { changed, userId, role: requestedRole } };
    }
    if (requestedRole === "none" || requestedRole === "remove" || requestedRole === "revoke") {
      const changed = await clearStoredUserAccessLevel(config, userId, { username, updatedBy: task.source.requesterUserId });
