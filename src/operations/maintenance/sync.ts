...
import { listRules, removeRule, sameRuleSlot } from "operations/context/rules-store";
import type { RuleRecord } from "operations/context/store";
import { resolveChat, resolveUser } from "operations/context/store";

export type SyncTask = {
  repoRoot: string;
  subject: "user" | "chat" | "rule-slot";
  operation: "refresh" | "dedupe";
  selector: {
    userId?: string;
    chatId?: string;
    topic?: string;
...
function taskKey(task: SyncTask): string {
  ...
  if (task.subject === "rule-slot" && task.operation === "dedupe" && task.selector.topic && task.selector.appliesTo) {
    return `${task.subject}:${task.operation}:${task.selector.topic}:${JSON.stringify(task.selector.appliesTo)}`;
  }
...
function selectRuleWinner(rules: RuleRecord[]): RuleRecord | null {
...
async function processRuleSlotDedupe(task: SyncTask): Promise<void> {
  const { topic, appliesTo } = task.selector;
  if (!topic || !appliesTo) return;
  const rules = listRules(task.repoRoot).filter((rule) => sameRuleSlot(rule, { topic, appliesTo }));
  if (rules.length <= 1) return;
  const winner = selectRuleWinner(rules);
  if (!winner) return;
  let removed = 0;
  for (const rule of rules) {
    if (rule.id === winner.id) continue;
    if (removeRule(task.repoRoot, { id: rule.id })) removed += 1;
  }
  if (removed > 0) {
    await logger.info(`sync removed ${removed} duplicate rule(s) for topic=${topic}`);
  }
}
...
async function processTask(task: SyncTask): Promise<void> {
  ...
  if (task.subject === "rule-slot" && task.operation === "dedupe") return processRuleSlotDedupe(task);
}
