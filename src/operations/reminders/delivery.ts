    ...
    for (const instance of dueInstances) {
      ...
      if (!preparedMessage && renderMessage) {
        try {
          ...
          await logger.warn(`reminder render fallback event=${activeEvent.id} notification=${instance.notificationId} error=${error instanceof Error ? error.message : String(error)}`);
