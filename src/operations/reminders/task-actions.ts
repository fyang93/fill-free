...
async function resolveReminderForUpdate(config: AppConfig, task: TaskRecord): Promise<ReminderEvent | null> {
  ...
  const candidates = active.filter((event) => {
...
export async function runReminderTask(config: AppConfig, task: TaskRecord): Promise<{ changed?: boolean; reminderId?: string; skipped?: boolean; reason?: string }> {
  if (task.operation === "create") {
    ...
    const event = await createReminderEventWithDefaults(config, {
      ...
      category: payload.category === "special" ? "special" : payload.category === "routine" ? "routine" : undefined,
      specialKind: payload.specialKind === "birthday" || payload.specialKind === "festival" || payload.specialKind === "anniversary" || payload.specialKind === "memorial" ? payload.specialKind : undefined,
      kind: payload.kind === "routine" || payload.kind === "meeting" || payload.kind === "birthday" || payload.kind === "anniversary" || payload.kind === "festival" || payload.kind === "memorial" || payload.kind === "task" || payload.kind === "custom" ? payload.kind : undefined,
