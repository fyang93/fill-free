...
export async function prepareReminderDeliveryText(config: AppConfig, agentService: AiService, event: ReminderEvent, now = new Date()): Promise<boolean> {
  const nextInstance = nextPendingReminderInstance(event, now);
  if (!nextInstance) {
...
export async function prewarmReminderDeliveryTexts(config: AppConfig, agentService: AiService): Promise<void> {
