...
import type { PendingAuthorizationDraft } from "support/ai/types";
import { persistState, rememberPendingAuthorization, consumePendingAllowedAuthorization, pruneExpiredPendingAuthorizations } from "scheduling/app/state";
import { setStoredUserRole } from "operations/access/roles";
...
export async function grantPendingAllowedAccessIfMatched(config: AppConfig, user: { id?: number; username?: string } | null | undefined): Promise<{ granted: boolean; username?: string; changed?: boolean }> {
  ...
  const changed = await setStoredUserRole(config, userId, "allowed", {
