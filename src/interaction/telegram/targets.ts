...
import { buildStructuredContextLines, resolveUser } from "operations/context/store";
import { listAuthorizedUserIds } from "operations/access/roles";
...
export function resolveTelegramTargetUser(config: AppConfig, rawTarget: unknown, ctx: Context, requesterUserId?: number): TelegramTargetResolution {
  ...
  const matchedChats = findTelegramChats({ id: directId, username }).filter((chat) => chat.type !== "private");
  ...
  const matchedUsers = findTelegramUsers({ id: directId, username }, authorizedUserIds);
