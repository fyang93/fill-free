...
import { loadChats, loadUsers, resolveChat, resolveUser } from "operations/context/store";
...
export function findTelegramUsers(input: { id?: number; username?: string }, allowedUserIds?: number[]): KnownTelegramUser[] {
  const allowed = allowedUserIdSet(allowedUserIds);
  ...
  const candidates = [input.username]
...
export function findTelegramChats(input: { id?: number; username?: string }): KnownTelegramChat[] {
  if (typeof input.id === "number" && Number.isInteger(input.id)) {
