...
export function buildTelegramReplyContextBlock(ctx: Context): string {
  ...
  if (reply.kind === "reply_to_message") {
    if (!reply.text) {
      return [
        "Reply context:",
        `The user is replying to a message from ${reply.senderLabel || "unknown"}. A reply target was available, but no text or caption content was included in the replied message payload.`,
        ...(reply.mediaSummary || []),
      ].join("\n");
    }
    return [
