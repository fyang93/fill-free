...
- `allowed user`：可以和 bot 对话、提问、查询非敏感信息，也可以要求 bot 为自己创建提醒；但仓库里的记忆/文件仍应视为受隐私限制、且实际上是只读的。不能读取或导出长期私密记忆、已有提醒详情、个人文件、密钥等敏感仓库数据；也不能上传/处理文件、修改长期记忆或仓库文件，或修改运行时配置。
...
- `allowed_user_ids`：允许和 bot 对话、但属于“受隐私限制”权限的 Telegram 用户 ID 列表。可以问一般问题，也可以为自己创建提醒；但不应读取私密记忆、已有提醒详情、个人文件或密钥等敏感数据，也不能上传/处理文件或修改仓库持久化数据。如果你只使用 trusted/admin，也可以留空。
...
- 过期的一次性提醒会在启动时自动清理；
- 空闲入梦只有在真的改动了提醒、tmp 清理或记忆文件时，才会给 admin 发送 Telegram 摘要；如果什么都没变，就不会通知；
