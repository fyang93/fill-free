...
┌──────────────────────┐   ┌──────────────────────┐   ┌──────────────────────┐
│ 🧹 维护器            │──>│ 📦 事务              │──>│ 💾 档案              │
│ 清理、修复与维护     │   │ reminders/access/... │   │ 规范持久状态         │
└──────────────────────┘   └──────────────────────┘   └──────────────────────┘
...
- **快路 / 响应器** 使用更窄的注入上下文，优先追求首字响应时间
- **慢路 / 执行器** 会在同一任务上并发启动，并可以直接产出直接回答、澄清问题，或带执行结果支撑的最终回复
- **仲裁器** 会优先发布最先得到的安全用户可见结果
- 如果快路判断为 `needs-execution`，慢路会继续，并且可以直接发最终回复
- 主回复流程里不再保留单独的回调回复阶段
...
当前回归测试覆盖了确定性的存储行为和真实自然语言流程，包括 reminder CRUD、rules 持久化、access role 变更、memory keywords 倒排索引、maintainer 的 keywords 回填，以及带时区换算的 reminder 时间表达。
