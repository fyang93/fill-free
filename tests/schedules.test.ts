...
import { buildScheduleEvent, createScheduleEvent, pruneInactiveScheduleEvents, readScheduleEvents } from "../src/bot/operations/schedules/store";
import { getCurrentOccurrence, normalizeRecurrence, scheduleEventScheduleSummary } from "../src/bot/operations/schedules";
import { buildScheduleScheduleFromExternal } from "../src/bot/operations/schedules/schedule_parser";
import { runScheduleTask } from "../src/bot/operations/schedules/task-actions";
import { prepareScheduleDeliveryText, clearPreparedScheduleDeliveryText, isPreparedScheduleDeliveryTextUsable, nextPendingScheduleInstance } from "../src/bot/operations/schedules/preparation";
...
describe("schedule task matching", () => {
  test("带具体年份的事件即使误传 yearly 也会落成一次性提醒", () => {
    const schedule = buildScheduleScheduleFromExternal({
      kind: "yearly",
      year: 2026,
      month: 4,
      day: 28,
  ...
  test("测试语句：删除 4/7 那个提醒，不应误删别的提醒", async () => {
    const config = await createTempConfig();
    const april7 = buildScheduleEvent(config, {
      title: "组会提醒",
      timeSemantics: "absolute",
      timezone: "Asia/Tokyo",
      schedule: { kind: "once", scheduledAt: "2026-04-07T06:00:00.000Z" },
    ...
    const april8 = buildScheduleEvent(config, {
      title: "组会",
      timeSemantics: "absolute",
      timezone: "Asia/Tokyo",
    ...
    await createScheduleEvent(april7, config);
    await createScheduleEvent(april8, config);
  ...
  test("删除提醒支持按 id / scheduleId 精确匹配", async () => {
    ...
    const recreated = buildScheduleEvent(config, {
      title: "4月28日组会",
      timeSemantics: "absolute",
      timezone: "Asia/Tokyo",
      schedule: { kind: "once", scheduledAt: "2026-04-28T01:00:00.000Z" },
    ...
    await createScheduleEvent(recreated, config);
  ...
  test("已过时但仍 active 的一次性提醒不会被启动/maintainer 清理提前删掉", { timeout: 15000 }, async () => {
    ...
    const result = await pruneInactiveScheduleEvents(config);
...
describe("schedule delivery text pipeline", () => {
  ...
  test("text is generated with the exact notifyAt timestamp of the target notification instance", async () => {
    const config = await createTempConfig();
    ...
    expect(event.deliveryPreparedNotificationId).toBe("n1");
    // The AI was called with the exact notifyAt timestamp
    expect(calls).toHaveLength(1);
    expect(calls[0]!.scheduledAt).toBe("2026-05-01T12:00:00.000Z");
  });

  test("cached text is only usable for the matching notification instance", async () => {
    const config = await createTempConfig();
    const event = buildScheduleEvent(config, {
      title: "开会",
      timeSemantics: "absolute",
    ...
    // Simulate text prepared for the pre-notification (24h before)
    event.deliveryText = "明天9点有会议，提前提醒";
    event.deliveryTextGeneratedAt = new Date().toISOString();
    event.deliveryPreparedNotificationId = "n-1d";
    event.deliveryPreparedNotifyAt = "2026-05-01T09:00:00.000Z"; // 24h before

    const preInstance = { notificationId: "n-1d", offsetMinutes: -1440, notifyAt: "2026-05-01T09:00:00.000Z" };
    const nowInstance = { notificationId: "n-now", offsetMinutes: 0, notifyAt: "2026-05-02T09:00:00.000Z" };

    // Pre-notification text IS usable for its own instance
    expect(isPreparedScheduleDeliveryTextUsable(event, preInstance)).toBe(true);
    // Pre-notification text is NOT usable for the on-time notification
  ...
  test("text is regenerated when schedule changes (cache is cleared)", async () => {
    ...
    event.deliveryPreparedNotificationId = "n1";
    ...
    expect(event.deliveryPreparedNotificationId).toBeUndefined();
  ...
  test("delivery: prepared text is usable and replaces fallback when notifyAt matches", async () => {
    ...
    const instance = { notificationId: "n1", offsetMinutes: 0, notifyAt: pastTime };
    ...
    const otherInstance = { notificationId: "n1", offsetMinutes: 0, notifyAt: new Date(Date.now() + 3600_000).toISOString() };
  ...
  test("schedule-change via update task causes stale cache — text not usable for new schedule", async () => {
    ...
    event.deliveryTextGeneratedAt = new Date().toISOString();
    event.deliveryState = { currentOccurrence: { scheduledAt: "2026-06-01T09:00:00.000Z", sentNotificationIds: [] } };
    await createScheduleEvent(event, config);
    ...
    expect(updated?.deliveryPreparedNotificationId).toBeUndefined();
