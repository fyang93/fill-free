...
- user-facing assistant replies follow the user's natural conversation language, while `/language` only controls fixed UI text
...
- `cli-schedules`: reminders and scheduled-task workflows
...
- `bot.language`: default UI locale used when a user has not selected `/language` yet; choose `zh-CN` or `en`
...
- `trusted user`: may read and modify memory, upload/process files, create schedules, and use other persistent workflows
